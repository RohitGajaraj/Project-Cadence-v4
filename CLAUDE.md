...
1. [`AGENTS.md`](./AGENTS.md) — pre-action protocol, engineering rules, escalation, session-friction loop, founding principles.
   ...
   1.65. **Build-in-public brand system: moved to a separate PRIVATE repo** (`RohitGajaraj/build-in-public`), split out 2026-06-15 so the founder's personal brand, voice, drafts, and social tokens never live in this product repo (which may be shared). **Standing rule (one-way insight feed):** when a non-obvious, postable build insight surfaces here, append it to [`docs/brand-feed.md`](./docs/brand-feed.md) (the single capture file the build-in-public engine reads first, before scouting and drafting). Keep it public-safe, no secrets. Never post to the founder's accounts without his explicit approval. Do not recreate `docs/brand/` in this repo.
