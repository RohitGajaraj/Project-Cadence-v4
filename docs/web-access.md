...
| `web.crawl` | read | `confirm` | Bounded crawl (max 25 pages, depth 2). Spends real Firecrawl credits — that's why it asks for approval. |
...
- **Untrusted input.** Returned markdown is treated as untrusted text. The *next* `callModel()` in the loop runs it through the same pre-guardrails (PII / prompt-injection / secret / keyword) as any user input — see [`architecture/runtime.md`](../architecture/runtime.md). Do not strip that step when extending these tools.
...
- [`feature-backlog.md`](./feature-backlog.md) — live status of this feature.
