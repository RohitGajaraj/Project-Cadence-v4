...
- **Checkpoints** — `agent_run_checkpoints` upserted on every loop iteration *before* the provider call (`src/lib/ai/loop.server.ts`).
...
| Mission flips `failed` after restart | Loop tried to call an external tool whose result wasn't yet checkpointed | Confirm checkpoint happens *before* the provider call (`loop.server.ts`) |
