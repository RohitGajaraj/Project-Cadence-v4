...
| Field | Current |
|---|---|
| 🔨 **Now building** | **— idle —** Mission page now shows live agent timeline + per-hop progress (steps + tool spans) with 2s refresh while running, plus a Trace link out of each hop. Web access (Firecrawl `web.*`) also stabilized: fixed `web.search` response-shape bug (`{data:{web,news}}` vs array) that was halting agent loops on step 1. Ready to start Bundle 5 (E6 Mission Graph DAG view). |
