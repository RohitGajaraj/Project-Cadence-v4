...
| Field | Current |
|---|---|
| 🔨 **Now building** | idle. **`F-IA-CALENDAR-MERGE` + `F-IA-BRIEFING-SETTINGS` shipped this turn** — pinned rail trimmed 6 → 4 (Today · Approvals · Calendar · Chat); `/meetings` and `/meetings/$id` redirect into a `/calendar` list-default surface with a side sheet for transcript/extract; `/briefing` redirects into Settings → Strategic brief (inline). |
...
**Last updated:** 2026-06-06 · Lovable (`F-IA-CALENDAR-MERGE` + `F-IA-BRIEFING-SETTINGS` shipped — pinned rail goes 6 → 4; Calendar absorbs Meetings; Briefing moves to Settings).
