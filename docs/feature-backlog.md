...
| Field | Current |
|---|---|
| 🔨 **Now building** | idle. **`F-CALENDAR-PERUSER` shipped this turn (Parts B + C)** — per-user Google + Microsoft calendar OAuth wired end-to-end, month grid + inline event create/edit/delete; gated only on two backend secrets (see [`./decisions/calendar-oauth-credentials.md`](./decisions/calendar-oauth-credentials.md) + `active-task.md`). |
...
**Last updated:** 2026-06-06 · Lovable (Calendar header polish — dropped connect banner + duplicate month/year dropdowns, added live weather chip with browser geolocation; calendar accounts now also manageable in Settings).
