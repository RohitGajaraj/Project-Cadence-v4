...
| Field | Current |
|---|---|
| 🔨 **Now building** | **— idle —** Bundle 6 lifecycle-close shipped — `github.issue.create` + `prd.link_issue` are now seeded for every existing user AND new signups (both at `confirm` mode), the tool handlers were already in `src/lib/ai/tools/registry.server.ts` from FND-RUNTIME 0.9 / Bundle 2, and `/prds/$id` now surfaces a clickable `Github · GitHub issue #N` chip when `prds.github_issue_url` is set. The Discover→Define→Plan slice (research.synthesize → prd.draft → backlog.prioritize → github.issue.create → prd.link_issue) is gate-ready end-to-end against the configured `GITHUB_REPO`. Forced-restart idempotency (re-running the same `idempotency_key` returns the cached issue, never double-creates) inherits from the FND-RUNTIME 0.9 `withIdempotency` wrapper. |
| ⏭️ **Next up** | Bundle 9 Build+Test — `github.pr.open` against this repo + CI read into the Mission Graph (I-thin, J-thin). Then Bundle 10 (Ship/deploy webhook into Mission Graph), 11 (Launch/changelog + one outbound channel), 12 (Support→Learn + Analyst re-score) — the lifecycle loop closes at Bundle 12. |
| 🚧 **Blocked / stuck** | — none. |
| 📊 **Progress** | Step **6 of 12** ✅ Bundle 6 lifecycle-close shipped — Discover→Define→Plan slice can now end in a real GitHub issue + PRD link-back via gated `github.issue.create` + `prd.link_issue`. Step 1 forced-restart test still ◑ (manual smoke; idempotency is wired). **Proof-platform v1.1 overlay:** bundle 1 ◑ · bundles 2–6 ✅ · bundles 9–12 ☐. |
