...
| Field | Current |
|---|---|
| 🔨 **Now building** | **F-AGENT-2 · Lovable** — Agent self-reflection + persistent memory + trust auto-advance **shipped**. New `autoReflect()` helper runs after every clean completion (both fresh + resumed runs); distils a strict-JSON `{lesson, what_worked, what_to_change, importance}` via Gemini Flash and writes a `kind='reflection'` row to `agent_memory` with embedding + metadata. `recallMemory()` now merges semantic matches with the top recent reflections (importance-ranked). Two new tools in the registry: `memory.reflect` (operator/agent on-demand reflection) and `memory.promote` (escalate memory scope agent→workspace). New `auto_advance_agent_arc(p_user_id, p_agent_id)` RPC moves Observing→Proving at 5 clean completed runs, Proving→Trusted at 20, never downgrades, blocked by any rejected approval since the last arc change; called from both finalize paths. New `recent_agent_reflections(...)` RPC + `listAgentReflections` server fn powers a "Recent reflections" panel on `/agents` showing the lesson + what-worked / what-to-change + importance. New `/api/public/hooks/memory-tick` daily cron deletes low-importance (≤2), unused (>30d) memories. Cron `memory-tick-daily` scheduled 03:30 UTC. `agent_memory` grants finally added (table was unreachable from PostgREST). |
| ⏭️ **Next up** | **F-AGENT-3** — event reactor + auto-pipelines: `signal.created`→Discovery (auto), `opportunity.scored`+`score>=80`→Strategist (confirm), `prd.status_changed`→`approved`→Orchestrator (confirm). Then **F-AGENT-4** (Swarm HUD). Restructure Phases 3–4 resume after the agent ecosystem is closed. |
| 🚧 **Blocked / stuck** | — none. |
| 📊 **Progress** | **Agent ecosystem bundle:** F-AGENT-1 ✅ · F-AGENT-2 ✅ · F-AGENT-3 ☐ · F-AGENT-4 ☐. **Restructure** ◑ Phases 1–2/4 shipped, 3–4 deferred. Bundle 9 ◑ Slice 1 done; Slices 2–3 paused. Step 1 forced-restart test still ◑. **Proof-platform v1.1 overlay:** bundle 1 ◑ · bundles 2–6 ✅ · bundle 9 ◑ · bundles 10–12 ☐. |

**Last updated:** 2026-06-06 · Lovable (F-AGENT-2 shipped — reflection + autonomy auto-advance live).
