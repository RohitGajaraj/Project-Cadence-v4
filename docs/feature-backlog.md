...
| Field | Current |
|---|---|
| 🔨 **Now building** | idle. **`F-IA-MERGE-OBSERVE` shipped this turn** — sidebar `AI Ops` renamed to `Run` (Observe · Evals); `/analytics` + `/traces` + `/drift` merged into `/observe` with tabs (Analytics · Traces · Drift), live count badges on Traces and Drift, old URLs 301-redirect to the matching tab. Govern group untouched. |
| ⏭️ **Next up** | `F-COCKPIT-MERGE`. Operator-only: run the FND-RUNTIME playbook to flip foundation row 0.9 to ✅. |
| 🚧 **Blocked / stuck** | — none. |
| 📊 **Progress** | **Agent ecosystem bundle:** F-AGENT-1 ✅ · F-AGENT-2 ✅ · F-AGENT-3 ✅ · F-AGENT-4 ✅. **Restructure** ◑ Phases 1–2/4 shipped, 3–4 deferred. **Bundle 9** ✅. **Step 1 FND-RUNTIME 0.9:** substrate ✅ · playbook ✅ · forced-restart proof ◑ (operator-runnable). **v3 audit triage:** ✅ shipped — 22 F-IDs minted (8 P0 / 11 P1 / 3 P2) + `F-NAV-ACCORDION` ✅ + `F-IA-MERGE-OBSERVE` ✅. **P0 voice + governance batch:** ✅ all 8 shipped. **Proof-platform v1.1 overlay:** bundles 1–6/9/10/11/12 ✅ via `F-OUTCOME-SURFACE`. **Security:** `F-SEC-REALTIME-RLS` open; `prompt_runs_ws_write` user-id check shipped. |

**Last updated:** 2026-06-06 · Lovable (`F-IA-MERGE-OBSERVE` shipped — Run group + `/observe` tabs replace AI Ops sprawl; Govern intact; old URLs redirect with `?tab=` preserved).
