...
[`../../plan.md`](../../plan.md) section 2 carries the _product_ features; this doc carries the _cross-cutting non-functional_ requirements. The build order ([`../../plan.md`](../../plan.md) section 3) should pull P0 items into the foundation (especially agent blast-radius, cost controls, injection defense, provider fallback, and runtime durability) — they are architecture, not afterthoughts. The SSOT task queue ([`SOURCE-OF-TRUTH.md`](./SOURCE-OF-TRUTH.md)) and the F-ID scope ledger ([`archive/feature-backlog.md`](archive/feature-backlog.md)) point back here. Revisit this review whenever scope expands.
