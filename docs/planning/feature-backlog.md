...
| Field                  | Current                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🔨 **Now building**    | — idle — `F-CRITIC-AGENT` + `F-SCRIBE-CITATIONS` ✅ shipped (M1 Golden Path middle hops). Critic runs inline on every new opportunity + freshly generated PRD; verdict chip on opportunity rows and PRD header opens a side sheet with risks, kill criteria, missing evidence, and a re-run button. `generatePrd` now retrieves up to 8 RAG chunks, asks Gemini for inline `[n]` citations, and persists the chunk list to `prds.citations`; PRD detail page renders a Citations card that deep-links to source signals/docs/meetings. |
| ⏭️ **Next up**         | Remaining M1 Golden Path gaps: **Slack connector** (full app-user OAuth + Scout ingest); **PRD → engineering task graph** (planner gap; today `mission.plan` plans agents, not engineering tasks); **Release notes draft + outcome card seeding** (`release` agent has no `generateReleaseNotes` fn; outcome card is read-only rollup). |
