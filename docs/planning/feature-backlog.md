...
**Last updated:** 2026-06-12 · Lovable — live published app fix: public backend env injection restored for the browser bundle; republish queued.

**Recent log** (newest first; trim to ~5 — full history lives in [`../plan.md`](../../plan.md) §4):

- `2026-06-12` — **Live published app fix.** Browser console on `https://cadence-flow-beta.lovable.app/chat` showed the app crashing before auth with missing public backend URL/key values. `vite.config.ts` now injects those public values from the build environment so protected routes can initialize, redirect, and render normally after republish.
