...
**Last updated:** 2026-06-12 · Lovable — preview server restarted; live published app fix corrected so public backend env injection is delegated to the framework config.

**Recent log** (newest first; trim to ~5 — full history lives in [`../plan.md`](../../plan.md) §4):

- `2026-06-12` — **Live published app fix.** Browser console on `https://cadence-flow-beta.lovable.app/chat` showed the app crashing before auth with missing public backend URL/key values. The manual public-env `vite.define` override was removed so `@lovable.dev/vite-tanstack-config` owns injection again; preview `/chat` now reaches auth, server fns, and backend requests without the missing-config crash.
