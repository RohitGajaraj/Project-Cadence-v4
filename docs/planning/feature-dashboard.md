...
| F-AGENTS-MENTIONABLE | Agents as first-class @-mentionable users | ✅ (2026-06-18) | Type `@agentslug goal` in chat to dispatch specialist directly (skips orchestrator). Parses mentions, creates single-step DAG, dispatches via `advanceMissionCore`. Completion deterministic via steps.length > 0. | Mention parser → mission dispatch (chat.ts) |
