...
> **Live progress (derived from the rows, recompute on every status change - see the Progress accounting rule above).** **Last recomputed: 2026-06-19 20:10 IST** from the register (the per-status counts and per-category roll-up are tallied, not hand-kept).

- ### Overall completion: **42% (strict) / 56% (weighted)**
  - **Strict** = fully-done rows / total = **61 / 145 = 42%** (the honest floor).
  - **Weighted** = sum of every row's Item % / total = **81.80 / 145 = 56%** (✅ 100%; ◐ partial at its per-row `[~NN%]` estimate; ⏸️ paused + 🔨 in-dev 50%).
  - **Remaining: 58% open** - **84 of 145** rows not fully done.
...
- **Total features = 145** · **Done = 61** · **Not done = 84** (⬜ 42 open + ◐ 29 partial + ⏸️ 3 paused + ⏭️ 10 deferred + 🔨 0 in dev + 🚧 0 blocked).
- _The Monetization + Credit + BYO lanes (G10/G11) are the bulk of what is open - **43 of the 84** not-done rows._
...
| --- | --- | --- | --- |
| ✅ Done | 61 | 100% | 61.0 |
| ◐ Partial | 29 | per-row `[~NN%]` (avg ~66%) | 19.30 |
| ⏸️ Paused | 3 | 50% | 1.5 |
| 🔨 In Dev | 0 | 50% | 0.0 |
| ⬜ Open (ready to pick up) | 42 | 0% | 0.0 |
| ⏭️ Deferred | 10 | 0% | 0.0 |
| 🚧 Blocked | 0 | 0% | 0.0 |
| **Total** | **145** | - | **81.80 / 145 = 56%** |
