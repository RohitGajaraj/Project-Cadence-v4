...
2. **The design-craft skills (consult for taste + interaction):** `impeccable` (design quality + the AI-slop bans: no identical card grids, no hero-metric template), `emil-design-eng` (Emil Kowalski's interaction-craft: easing, timing, restraint), and the taste skills (`gpt-taste`, `design-taste-frontend`, `make-interfaces-feel-better`). **Founder ruling 2026-06-20: actively INVOKE the relevant design skills (and any fitting design-review agent) on EVERY front-end build, not merely reference them, so the product is right upfront instead of corrected later. Full authority to use whatever skill or agent fits.** (E.g. cycle 51 ran `impeccable` against the new Settings card before shipping.)
