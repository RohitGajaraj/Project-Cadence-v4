...
| [`screenshot-placement.md`](./screenshot-placement.md)   | **No images at the repo root or `docs/` top level.** Every capture has one logical home by scenario (verify / app-ui / reference / screen-* / brand-feed / committed design-reference) plus a retention window; `scripts/clean-screenshots.sh` (run on session Stop) relocates strays and purges ephemeral buckets so the tree self-heals. Founder ruling 2026-06-16. |
...
2. **Reference it from the canonical contracts** that already touch the topic — `architecture/*.md`, `design.md`, `docs/feature-backlog.md`. The contract restates the rule and links here for the _why_.
