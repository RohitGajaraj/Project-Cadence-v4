...
- **Building a feature, making a UX/positioning decision, or onboarding?** Read **v7 (Agentic Product OS, the reset)** first. It is the current positioning + build canon (code-verified state, four course-corrections, dual-persona, proof-gated M-0→M-D roadmap; founder rulings in §13). v6 holds the prior canon + engine/IA detail + market appendices; v5 holds the wedge UX detail; v4 governs expansion scope (stations, mesh, M2 to M5); personas in v3. Superseded iterations live in [`archive/`](./archive/).
