...
**Course-corrections committed (founder, 2026-06-14):** (1) **moat = compounding memory**, not orchestration (commoditizing); (2) **ambient + governed**, not "autonomous" (reframes observing-default as a positioning/UX fix, not only a build gap); (3) **hybrid + outcome pricing** — charge for memory+decisions; the >$150/mo team bar is a *hypothesis to validate*, not a set price; (4) **complete the loop on real data before breadth.** Beachhead = **dual-persona** (senior/founding PM @ 50–400 B2B SaaS + individual PM/prosumer); GTM sequenced **serial-with-overlap** (P2 PLG first → P1 design partners) to avoid the two-heavy-motions trap the canon itself warns against.
...
### 2026-06-14 — KI-14: standardize the eval score scale on 0–100 (decision + the "verify recon against source" lesson)

**Decision:** The suite-eval score scale (`eval_runs.avg_score` / `eval_case_results.score`) is standardized on **0–100**, not 0–1. Migration-led: widen `numeric(4,3)`→`numeric(6,3)` + rescale existing 0–1 rows ×100 across `eval_runs`, `eval_case_results`, and the derived `drift_snapshots.avg_eval_score`. The only app change is a drift UI formatter (render the 0–100 integer).

**Why 0–100:** it's the live judge's native scale (`eval-runner.server.ts` prompts "0-100" and clamps to it), the scale of `eval_suites.pass_threshold` (INTEGER, default 70 / seeded 80), and what the eval UI already assumes — so it needs the fewest changes. The column was the only thing wrong: `numeric(4,3)` (max 9.999) couldn't hold a real 0–100 write (silent overflow) and made the 0–1 seeds read "score 1 · below gate 80". 0–1 would instead have meant changing the judge prompt, the threshold semantics, the form, and the display.

**The method lesson:** both recon agents flagged `trust.server.ts` and `drift.server.ts` as mandatory consistency edits ("trust wraps to max", "drift breaks"). Reading the actual source disproved both: trust consumes a DIFFERENT table (`ai_evals`, the per-message chat-judge, still 0–1) and drift compares via a scale-invariant pct-delta. Acting on the recon's word would have introduced two needless, possibly-breaking edits. **Verify recon claims against source before coding** — recon points you at the files; it doesn't get a vote on the diff.

**Tradeoffs considered:** Reproducing the 354-line `seed_demo_workspace` to fix its six 0–1 literals (rejected — it's untestable here, and a transcription error would silently break the official investor-demo provisioner; the review's "it's a 5-number edit" is mechanically wrong since `CREATE OR REPLACE` needs the whole body). Instead the documented re-seed (`demo-credentials.md`) now warns + ships the corrected procedure — fix the operator path, not just the code path. `numeric(5,2)` vs `numeric(6,3)` (chose 6,3 — keeps the old 3-decimal scale, so the widen loses no precision). A `CHECK(0..100)` constraint (skipped — `numeric(6,3)` already can't overflow for 0–100, and an unseen out-of-range live row would fail the migration apply).

**Impact:** Migration `20260614160000_p3_ki14_eval_score_0to100.sql`; the `DriftPanel`/`DriftSurfaceDetail` formatter; docs (`demo-credentials.md` re-seed, `known-issues.md` KI-14 ✅, the eval-gate threshold restated 0–100 across AGENTS/architecture/plan/feature-backlog). Hardened by a 3-lens adversarial review (12 findings, none blocking). Applies on the next Lovable sync. **Next: the shareable-decision viral loop + pricing, gated on the §8 gauntlet.** Build-log: [`../../plan.md`](../../plan.md) §4 (2026-06-14 · KI-14).
