...
**Why:** per-workspace billing taxes the moat (more workspaces = more compounding memory); account-level pooling is the flywheel. Memory is both the moat and the lock-in, so scoping it correctly is a security and a monetization decision at once. Reconciled with the parallel credits/monetization session and [`byo-build-and-cadence-cloud-2026-06-18.md`](./byo-build-and-cadence-cloud-2026-06-18.md) §5.5 (the monetization canon). Naming kept brand-independent because the product name is a placeholder.
