...
> Status · Shipped 2026-06-14 (ritual_sessions migration applies on next Lovable sync) · Route `/govern?tab=gauntlet` · Owner: the loop (no single agent)

## What it does

A read-only tab in Govern that surfaces the three north-star proof metrics on a real surface: **Acceptance rate** (how often you approve the loop's calls), **Autonomy ratio** (how much side-effecting work the loop runs unattended vs gates for you), and **Ritual retention** (how many days you opened Today to clear the queue). Each card shows a headline number, a trend direction, and one plain-language line of what it means. Every number is computed from real tables — when a window is sparse the card says "not enough data yet" instead of inventing a figure.
...
- `bun run build` is green; `bun test src/lib/gauntlet-metrics.test.ts` passes (9 tests).
