...
- **Four tools** in `src/lib/ai/tools/orchestrator.server.ts`, wired into `src/lib/ai/tools/registry.server.ts`:
  - `mission.plan` — calls a sub-model with the user's actual specialist roster, asks for strict JSON `{summary, steps:[{agent_slug, sub_goal, depends_on, rationale}]}`, 1–6 steps cap, validates every slug.
