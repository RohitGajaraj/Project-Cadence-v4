...
- **DBR-3:** the Brain surface becomes the **visual graph** (Obsidian-style explorer) plus provenance, freshness, and governing-decision retrieval. Absorbs `O3` (fact drift + skill packs).
  ...
  - **DBR-3c (SHIPPED 2026-06-22, ◐ dormant):** completes the literal promise "return the governing DECISION, not the nearest text" by NAMING the replacement. The Critic block and the precedent nudge now read "replaced by 'New checkout flow'" instead of citing an opaque id - a fail-safe server resolver (`attachGoverningTitles`) batch-looks-up the governing decision's title from `prds`/`opportunities` and threads it through the shared `resolveGoverningForNodes`, so both surfaces gain the name with no extra wiring. Id-only fallback when a title cannot resolve.
  - **DBR-3d (SHIPPED 2026-06-22, ◐ dormant):** governing-decision so far flagged stale PRECEDENTS (similar past decisions). This adds the complementary, previously-missing piece: it flags the decision the user is VIEWING if IT has itself been superseded or contradicted, so a user never unknowingly acts on a stale decision. A `getDecisionCurrency` server fn resolves the viewed entity's own currency (the same `resolveGoverningForNodes`, applied to the entity's own id), and a fail-safe `DecisionCurrencyBanner` sits above the precedent nudge on the opportunity + PRD surfaces ("This decision was superseded by 'X'. Act on the current one, not this."). Renders nothing until the entity is graph-marked stale.
  - **DBR-3e (SHIPPED 2026-06-22, ◐ dormant):** the 5th "value at every step" moment (chat / Ask). The chat answer path now loads decision precedent for the user's message and injects a `formatDecisionPrecedent` block into its system prompt, so the assistant grounds in the workspace's OWN outcome history ("you shipped a similar bet and it missed") rather than generic advice. Threshold-gated (conservative 0.3), fail-safe, byte-identical when no outcome matches, and guarded (a min-length floor so trivial messages skip the embed; a passive-text / no-injection rider; the verdict tags are flagged as labels, not citations). Now four of the five moments are live (Critic, precedent nudge, decision-currency banner, chat).
  - **DBR-3f (SHIPPED 2026-06-22, ◐ dormant-with-no-data):** SHARED-PREMISE precedent - the multi-hop, graph-over-vectors query the evidence table above names ("what happened last time we reasoned this way?"). Where DBR-2/3 walk the OUTCOME-reversal edges (`supersedes`/`contradicts`), this walks the orthogonal DERIVATION graph: from the decision under review UP (child -> parent over `promoted`/`cites`/`derived-from`/`depends-on`) to its premise ancestors (the upstream signal/opportunity/theme it rests on), then back DOWN to those premises' OTHER descendants - the "cousins" that share a premise but sit on a different path - and reports each cousin PRD's recorded outcome. The signal flat vectors structurally cannot give: "the last PRD derived from this same opportunity MISSED", surfaced because they share a structural premise, not similar text. A pure two-directional walk + missed-first ranking + formatting (`src/lib/ai/shared-premise.ts`, 21 tests incl. a depth-complete subtree-exclusion regression); two bounded, fail-safe BFS closures + a `prds.outcome` join (`src/lib/ai/shared-premise.server.ts`); wired as an independent fail-safe block into `runCritic` (`src/lib/ai/critic.server.ts`). 3-lens adversarial review (correctness / runtime-safety / over-claim) = unanimous SHIP, 0 must-fix; folded the depth-complete exclusion (so the target's own deep descendant can never leak as a cousin) + an honesty softening of the block's framing. tsc 0 / 1102 tests. Byte-identical + fail-safe until derivation edges + recorded outcomes both exist. Next: extend the same resolver to the proactive precedent nudge (mirroring DBR-3b's Critic -> nudge fan-out).
  - **DBR-3g (SHIPPED 2026-06-22, ◐ dormant-with-no-data):** shared-premise precedent extended from the Critic (a review surface) to the PROACTIVE nudge ("deliver value at every step, before you ask"), mirroring DBR-3b's Critic -> nudge fan-out. The server resolver was refactored to a structured `resolveSharedPremiseItems` (one graph in, both surfaces out; `resolveSharedPremisePrecedent` is now its thin Critic-formatting wrapper, behavior-preserved); a new `getSharedPremisePrecedent` server fn (`src/lib/shared-premise.functions.ts`, RLS-scoped, fail-safe) feeds a new `SharedPremiseNudge` (`src/components/decision/SharedPremiseNudge.tsx`) mounted on the opportunity + PRD detail surfaces beside the existing precedent/currency nudges. Distinct from `PrecedentNudge` (text-similar past decisions): this shows decisions built on the SAME upstream premise and how each landed. Focused code-review = SHIP_WITH_FIXES; folded both fixes (a null-summary trailing-colon render; an `enabled: !!targetId` guard so the uuid-validated server fn never throws on an unresolved id). tsc 0 / 1110 tests. Byte-identical / renders nothing until the graph carries derivation edges + outcomes. IA note: three decision asides now stack on these surfaces (precedent, shared-premise, currency) - a candidate for the founder-prompted IA-consolidation pass.
