...
5. Trigger a pending approval (or use a seeded one) — it appears in the **Attention queue** with Approve/Reject buttons that succeed and clear the row.
...
8. Sign in as `demo2@redcadence.app` — the HUD shows only that workspace's agents/missions/queue rows (RLS holds).
