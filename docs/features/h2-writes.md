...
- **Richer write surface** — the audit-provenance half, its UI render, and bulk reprioritize shipped (see the increments below); autonomous agent-commit wiring is the remaining slice (chokepoint, founder-attended).
...
- **Bulk re-prioritize (Lane 1, 2026-06-21):** the multi-item analogue of the lenient drag, for re-sequencing a board ("push these three to Next") in one action. New pure `src/lib/roadmap-bulk.ts` `planBulkMove(ids, target, items, cap)` (de-dups the selection, drops unknown ids + no-ops so the audit gets no phantom rows, caps at `BULK_MOVE_CAP=200` and reports the overflow rather than silently dropping it; 10 unit tests) drives a new `bulkUpdateRoadmapItems` server fn: read prior state (`.in("id", ids).eq("user_id")`, RLS-scoped) → plan → one `.update({roadmap_bucket}).in(ids).eq("user_id").select()` → record a `move` audit per moved row via the SAME shared `classifyRoadmapWrite` (so a bulk move carries its real from-bucket + still-promised outcome, never a phantom commit), returning honest `{moved, skipped}`. Lenient like the drag — per-item outcome+measure governance still binds at `commitRoadmapItem` and the gap surface keeps flagging the moved items, so bulk-move is NOT a governance bypass. UI (`RoadmapBoard.tsx`): a subtle per-card multi-select checkbox (ember accent; stops click-propagation so selecting never starts a drag) and a calm bulk action bar that appears only once a set is selected (Engine-Room calm front) — `N selected · move to · Backlog/Now/Next/Later · clear`; optimistic, mirrors `mMove`, rolls back on error. Folded an adjacent consistency fix the review surfaced: both optimistic writers now spread `...old` so `governanceGaps` no longer flickers to 0 on a move. 6-agent adversarial review (correctness + security/tenancy + regression, each finding adversarially verified): **0 must-fix**. tsc 0 / eslint 0 / `bun test` 618 (10 new).
