...
> - **Engine + pure ranker: ✅ unit-verified.** `rankPrecedent` + `loadDecisionPrecedent` in `src/lib/ai/decision-precedent.server.ts` (3 unit tests; full suite 322 pass). Commits: T1 `4628526425`, T2 `7865107c95`, T3 `ed048fe534`, T4 `f424c72bf6`, T5 `526f156ced`, T6 `bf4a83a084`.
> - **The 3 seams: ◐, render-on-publish.** Critic (seam 1, `runCritic` now calls the semantic engine and the old DBR-0 recency helper is deleted), opportunity detail (`OpportunityDetail.tsx`, the `?opp=` drill), spec detail (`_authenticated.prds.$id.tsx`). They are wired + driven + gate-green, but NOT render-verified locally (they read live embedded outcome data, so behavioral verification needs the founder's next publish + seeded outcomes). UI breadcrumbs: an opportunity drill, a spec detail page, and the Critic verdict sheet.
> - **As-built deltas from this spec (intentional, recorded for honesty):** (1) the relevance threshold ships at `PRECEDENT_THRESHOLD = 0.3` (deliberately conservative, tune on live precision/recall after the first publish), NOT the spec's `~0.72` placeholder. (2) The primitive reuses the real `VerdictChip` Ember primitive (tone moss/ember/madder) rather than an invented chip-class map; verdict to tone is validated to moss, missed to madder, mixed to ember. (3) `excludeId` is plumbed but not passed from the v1 seams (a row can cite its own past outcome, which is acceptable reflection). (4) account-pooling is not applied to precedent in v1 (workspace-scoped, with the `match_agent_memory` PGRST202 deploy-window fallback). (5) Optional follow-up noted by the final review: skip rendering when the metadata fetch errors (currently degrades to a `mixed`/null-title row, within the fail-safe mandate).
