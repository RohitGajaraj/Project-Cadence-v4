...
- **No in-app voucher redeem surface.** `redeemVoucher` (`admin-vouchers.functions.ts`) has zero callers; the engine is admin-create-only. Build a "redeem a code" input (Account scope → Settings Credits) that calls it.
