...
### F-BUILDER-MULTIFILE: scoped multi-file build (touch list + max-files cap)

A Studio changeset can carry a **pre-declared touch list** (the only paths the operator sanctioned) and a **max-files cap**. The Changes tab evaluates the staged files against that policy and surfaces what is out of scope and over the cap, so a multi-file session stays inside its sanctioned blast radius before the confirm-gated commit. This is the thin slice of I1: a scope contract over the changeset, enforced at the operator layer (the agent's `studio.stage` tool is unchanged; the operator curates against the policy before approving the commit).

- **Where it lives.** `Build > session > Changes > Scope`. A "Scope" card sits above the file list: it shows the live status ("N files, all in scope", or "X outside scope · Y over the cap of M" in amber), an **Edit** control that opens a touch-list editor (one path per line) plus a max-files input, and an **Apply scope** button (staged changesets only) that drops every out-of-scope file in one action. Each out-of-scope file also carries an "out of scope" badge in the file list.
- **Touch-list matching** (`matchesTouchList`, `src/lib/ai/studio-hunks.ts`): exact path; directory prefix (an entry ending in `/`); or a glob (`*` within a segment, `**` across segments, with slash-bounded globstars collapsing zero or more segments, so `src/**` and `src/lib/*.ts` both work). The cap is a **warning, never an auto-drop** (which files to shed under the cap is the operator's call).
- **Pre-declaration.** `dispatchStudioSession` accepts optional `allowedPaths` + `maxFiles` and records them mission-keyed (so the policy is in place before the agent lazily creates the changeset). The operator can also set or clear it any time via `setChangesetConstraints`.
- **Data model** (additive migration `20260619270000_studio_multifile_constraints`): `studio_changeset_constraints (mission_id unique → missions, workspace_id, user_id, allowed_paths text[], max_files int, …)`, RLS mirrored from `studio_changesets` (workspace-member read; owner write). Missing-table reads degrade to an unconstrained policy (pre-sync window), so `getStudioSession` never errors before the migration applies.
- **Pure logic, unit-tested** (`studio-hunks.test.ts`): `matchesTouchList` (exact / prefix / single- and double-star globs), `evaluateFileSetPolicy` (in/out-of-policy split, over-cap count, clean flag), and `applyChangesetHunkSelections` (the changeset-wide "merge across files" primitive). The changeset-wide merge stays a pure helper rather than a server endpoint, since the operator path drives per-file curation via `applyStagedHunkSelection`; the agent/contract door imports the helper directly.
- **Server fns** (`studio.functions.ts`): `setChangesetConstraints` (declare/clear, mission-keyed), `enforceTouchList` (drop out-of-scope staged files; no-op when no touch list), and the `getStudioSession` policy report (`constraints` + `fileSetPolicy`). Only a `staged` changeset can be curated.
- **Status (2026-06-19):** built; tsc + lint + `bun run build` green, 24 hunks/policy tests pass. ◐ live-verify pending the founder's publish + the constraints migration syncing via Lovable (KI-08 pattern). Scope: operator-layer only — no chokepoint / loop / approval / governance changes.
