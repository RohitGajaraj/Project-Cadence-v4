...
> **Deploy ordering (important):** the route now selects `mcp_tokens.scopes` and calls `interop_write_enabled()`, so the migration MUST be applied with (or before) this code — they ship together on a Lovable publish. Until applied the read tools would error on the missing column. After applying, the gate stays OFF and nothing external can write until the founder flips it.
