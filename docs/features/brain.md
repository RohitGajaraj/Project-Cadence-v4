...
| --- | --- | --- |
| `web` | Current external facts — weather, news, prices, competitors | 1–3 focused sub-queries → **parallel** web searches (top 6 deduped sources, scraped) |
...
- **2026-06-12 — the day-one 401:** no prior chat UI version attached the `Authorization: Bearer` header — every request failed silently; root cause of "chat never responds" (fixed in `F-CHAT-V2`).
