...
- **Update the Live status board** at the top of [`docs/planning/feature-backlog.md`](./docs/planning/feature-backlog.md) — _Now building · Next up · Blocked · Progress · Recent log · Last updated_. This is the shared cursor every tool (Claude Code, Antigravity, Gemini, Lovable) reads to know where we stopped and what's next; leave it true at the end of every session, even a paused one.
