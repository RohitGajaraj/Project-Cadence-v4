-- Restore the full seed_demo_workspace function (the previous patch truncated its body)
-- and switch the hardcoded slug 'demo' to a per-owner slug so multiple demo users don't
-- collide on the global UNIQUE(slug) constraint.
...
BEGIN
  ...
  -- Per-owner slug so the global UNIQUE(slug) constraint doesn't block seeding
  -- when more than one user gets a demo workspace.
...
-- Clean up the demo accounts' empty default workspaces, then re-seed.
DO $$
DECLARE
  v_user_id uuid;
...
BEGIN
  FOREACH v_email IN ARRAY v_emails LOOP
    ...
    -- Remove any empty 'My workspace' rows (no child content) owned by this demo user
    -- so the seeder can run clean. Cascade deletes will remove the workspace_members row.
    DELETE FROM workspaces w
    WHERE w.owner_id = v_user_id
      AND w.name = 'My workspace'
      AND NOT EXISTS (SELECT 1 FROM projects WHERE workspace_id = w.id)
      AND NOT EXISTS (SELECT 1 FROM signals  WHERE workspace_id = w.id)
      AND NOT EXISTS (SELECT 1 FROM prds     WHERE workspace_id = w.id)
      AND NOT EXISTS (SELECT 1 FROM missions WHERE workspace_id = w.id);
