...
### Open conflicts logged for founder ruling (2026-06-11)

Recorded in [`docs/strategy/session-decisions.md`](./docs/strategy/session-decisions.md): (1) north star — "default workspace where product work happens" (PM-first) vs the v4 canon's "B2B Enterprise Product Cockpit — agents execute, humans govern"; (2) agent roster — the ~15 PM-domain agents named below vs the v4 19-agent mesh vs `plan.md` §6's shipped "6 durable + ephemeral" model; (3) the **June 22, 2026** production-quality MVP date vs the no-dates M1–M5 milestone doctrine (current working interpretation: June 22 = the M1 Golden Path at production quality); (4) "smallest viable category" vs full-lifecycle org scope. Until ruled: the v4 canon governs scope; this constitution governs posture and principles.
