...
**A. If you are INSIDE the lane worktree** (`$WT` ends in `cadence-lane-4` or `cadence-build`, AND `.remember/LANE.md` exists) - start the loop right here, in this VS Code integrated terminal:
...
2. Operate as a **continuous `/loop`** (Claude Code's native self-pacing loop; the founder may also start it by typing `/loop` with the cycle prompt). **Each iteration = one item:**
   ...
   - **CLAIM IT ATOMICALLY BEFORE ANY CODE:** `bash scripts/lane.sh claim <ID> 4 "<globs you will touch>" "<note>"`. If it prints `HELD` or `CONFLICT`, pick another item. A `CLAIMED` line is a precondition for writing code.
   - Flip the dashboard row to `🔨 In Dev (4, <date time>)` and push that one-line claim first.
...
**B. If you are NOT inside the lane worktree** (on `main`, etc.) - do NOT build and do NOT open a terminal window. Tell the founder to open Lane 4 in a VS Code integrated terminal (`Cmd+Shift+P` → Tasks: Run Task → **"Lane 4"**, or open `cadence-parallel.code-workspace` → right-click the Lane 4 folder → Open in Integrated Terminal) and start it there. Then stop.
