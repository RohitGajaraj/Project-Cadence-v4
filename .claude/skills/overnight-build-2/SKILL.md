...
**A. If you are INSIDE the lane worktree** (`$WT` ends in `cadence-lane-2` or `cadence-knowledge`, AND `.remember/LANE.md` exists) - start the loop right here, in this VS Code integrated terminal:
...
2. Operate as a **continuous `/loop`** (Claude Code's native self-pacing loop; the founder may also start it by typing `/loop` with the cycle prompt). **Each iteration = one item:**
   ...
   - **CLAIM IT ATOMICALLY BEFORE ANY CODE:** `bash scripts/lane.sh claim <ID> 2 "<globs you will touch>" "<note>"`. If it prints `HELD` or `CONFLICT`, pick another item. A `CLAIMED` line is a precondition for writing code.
   - Flip the dashboard row to `🔨 In Dev (2, <date time>)` and push that one-line claim first.
