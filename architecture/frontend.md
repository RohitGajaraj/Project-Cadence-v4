...
The pinned (always-visible) rail in `AppShell` holds **three** items: **Today · Approvals · Brain** — the surfaces touched every minute. `Approvals` deep-links to `/govern?tab=approvals` and carries a live pending-call badge; `Brain` is `/chat`. Below it the **Loop** group lists Product · Build · Missions · Knowledge · Memory. The Pin test (see [`../docs/conventions/inline-management.md`](../docs/conventions/inline-management.md#pin-test-applies-to-the-sidebars-pinned-workspace-rail)) governs what earns a pin. A **floating quick-access dock** (bottom-right, `z-50`, hidden on mobile) exposes **Calendar** → `/knowledge?tab=calendar` with a hover tooltip; Approvals is intentionally NOT in the dock. **Single-sourced Approvals (Phase-2 IA de-dup, 2026-06-16):** the sidebar-footer **Trust** row holds the governance shortcuts **Budgets · Engine Room · Connectors** only — Approvals was removed from it so the sidebar surfaces Approvals exactly once (the badged rail item). **Connectors** → `/sync` owns the workspace-level resource bindings + sync-conflict resolution; it is distinct from **Settings → Accounts** (the account-level OAuth provider logins, renamed from "Connections" in the same Phase-2 de-collision so the two near-identical names no longer clash). Calendar is also the **default tab of `/knowledge`** (see Knowledge section below), so the swarm has one substrate (events + learnings + decisions + docs) to pull threads from; the dock is the fast path, the Knowledge tab is the home. Everything else lives inside a collapsible group, inside a parent surface (tab / sheet / inline section), or in Settings.
