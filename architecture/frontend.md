...
`/govern` follows the same tabs pattern as the merged surfaces. One page, **ten tabs** with outcome-named labels per the Engine-Room Doctrine (the `?tab=<id>` ids are the routing contract and stay unchanged; only the display label changed, applied 2026-06-16): Controls (kill switch · mission caps · stuck approvals · reactor) · Approvals (tool-call queue) · **Safety** (id `guardrails`, content rules) · **Spend** (id `budgets`, spend caps) · Prompts (prompt studio) · **Quality checks** (id `evals`, eval suite) · Analytics (run metrics) · **Loop health** (id `gauntlet`, the proof metrics) · **Activity** (id `traces`, execution waterfall) · **Trends** (id `drift`, model drift). The surface header reads **Engine Room** (matching the Trust-row door); the route id stays `/govern`. See [`../docs/conventions/engine-room-doctrine.md`](../docs/conventions/engine-room-doctrine.md). Tab state is `?tab=controls|approvals|guardrails|budgets|prompts|evals|analytics|traces|drift` via `validateSearch`, default `controls`. Panel JSX lives in `src/components/governance/{Approvals,Guardrails,Budgets,Evals,Prompts}Panel.tsx`; Analytics, Traces, and Drift panels live in `src/components/observe/` and are reused here. The Controls panel stays inline in the route file because it depends on the same `getGovernanceOverview` + reactor server fns. Legacy routes `/inbox`, `/guardrails`, `/budgets`, `/evals`, `/prompts`, `/analytics`, `/traces`, `/drift`, and `/observe` are all `beforeLoad`-redirects to the matching Govern tab. Sidebar workspace rail's "Approvals" deep-links to `/govern?tab=approvals` — `NavRow` forwards a per-item `search` prop to `<Link>`, and `AppShell.isItemActive` checks the current `search.tab` so a path can host multiple correctly-highlighted nav entries. Govern group ships with two items only: Governance, Integrations.

## Pinned workspace rail

The pinned (always-visible) rail in `AppShell` holds **two** items: **Today · Ask** — the surfaces touched every minute. **`Ask`** is the chat surface (route stays `/chat`) that queries the Brain; it was renamed from "Brain" when the Knowledge surface took the name **Brain** (founder ruling 2026-06-16: the knowledge substrate owns the word "brain", the query interface is "Ask"). Below it the **Loop** group lists **Product · Build · Missions · Brain** — the old top-level **Memory** nav item was folded into Brain as its "Memory" tab (see Brain section below). The Pin test (see [`../docs/conventions/inline-management.md`](../docs/conventions/inline-management.md#pin-test-applies-to-the-sidebars-pinned-workspace-rail)) governs what earns a pin. A **floating quick-access dock** (bottom-right, `z-50`, hidden on mobile) exposes **Calendar** → `/knowledge?tab=calendar` with a hover tooltip. **Single-sourced Approvals (Phase-2 IA de-dup, founder ruling 2026-06-16):** Approvals appears exactly once in the sidebar — in the footer **Trust** row, deep-linking to `/govern?tab=approvals` and carrying a live pending-call badge (the count pop-up). It was removed from both the daily rail and the dock so it lives in the Trust governance cluster, where it reads best, without taking a rail slot. The **Trust** row is therefore **Approvals (badged) · Spend · Engine Room · Connectors**. **Connectors** → `/sync` owns the workspace-level resource bindings + sync-conflict resolution; it is distinct from **Settings → Accounts** (the account-level OAuth provider logins, renamed from "Connections" in the same Phase-2 de-collision so the two near-identical names no longer clash). Calendar is also the **default tab of `/knowledge` (Brain)** (see Brain section below), so the swarm has one substrate (memory + learnings + decisions + docs + events) to pull threads from; the dock is the fast path, the Brain Calendar tab is the home. Everything else lives inside a collapsible group, inside a parent surface (tab / sheet / inline section), or in Settings.
...
## Brain surface (Memory + Learnings + Decisions + Docs + Calendar)

`/knowledge` is the consolidated **Brain** surface — the product's brain, the one substrate of everything it knows (founder ruling 2026-06-16: Knowledge → **Brain**, positioned in copy as the "Product brain"; route stays `/knowledge`). One page, **five** tabs: **Calendar** (default — events + meeting transcripts in one feed) · **Memory** (the compounding agent-recall **moat** — what the loop wrote and reaches for again; this is the old top-level `/memory` surface folded in, rendering the same `<MemoryList />`, `agent_memory` data) · **Learnings** (the human-recorded outcome learnings, `MemoryPanel` over the `learnings` table) · **Decisions** (workspace decisions log) · **Docs** (workspace pages, Notion / Google Docs import). Tab state via `validateSearch` (`?tab=calendar|memory|learnings|decisions|docs`), default `calendar`. **Tab-id history:** the `memory` id now belongs to the agent-recall Memory tab; the Learnings tab (which briefly held `id=memory` after the Phase-2 de-collision) was re-id'd to `learnings`, and its deep links (`LearningDetail` back, `MemoryPanel` row open, Today's "What changed" tile) were re-pointed to `?tab=learnings`. The legacy `/memory` route is a `beforeLoad`-redirect to `/knowledge?tab=memory`. The Calendar panel preserves the meeting deep-link sheet via a sibling `?meeting=<id>` search param that the route forwards as a prop to `CalendarPanel`; the Learnings tab uses `?learning=<id>` for `LearningDetail`. Panel JSX lives in `src/components/knowledge/{Calendar,Memory,Decisions,Docs}Panel.tsx` (the Memory tab reuses `src/components/memory/MemoryList.tsx`). Calendar reuses `listCalendarEvents`, `listMeetings`, `syncCalendar`, `createCalendarEvent`, `updateCalendarEvent`, `deleteCalendarEvent`, `proposeSlots`, and the per-user connect server fns unchanged. Legacy routes `/calendar`, `/docs`, `/meetings`, `/meetings/$id` are `beforeLoad`-redirects into the matching tab.
