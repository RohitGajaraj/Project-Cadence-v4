...
- **Billing-secret vaults are grant-locked, not RLS-only** (`account_billing_secrets` / `workspace_billing_secrets`, the per-account/workspace Stripe customer + subscription linkage). They are deny-all (RLS on, 0 policies → service-role-only), AND every `anon`/`authenticated` table grant is revoked (migration `20260622080000`, completing the SELECT-only revoke in `20260622020000`) so they match the `connection_secrets` pattern. **Why grants too, not just RLS:** Postgres does **not** apply row security to `TRUNCATE` (or `REFERENCES`/`TRIGGER`) — only to SELECT/INSERT/UPDATE/DELETE — so a residual `TRUNCATE` grant is an RLS-independent integrity/availability vector. Secret/credential tables must therefore be locked at the grant layer as well. **Follow-up (SEC-TRUNCATE-HARDENING):** the same default `TRUNCATE`/`REFERENCES`/`TRIGGER` grants still sit on the RLS-policied credential/token tables (`user_api_keys`, `mcp_tokens`, `ingest_tokens`); not reachable via PostgREST (no TRUNCATE verb) so low practical risk, but a targeted `revoke truncate, references, trigger` would close the class without touching the RLS-gated owner paths.
...
**Governance surface (operator UI).** The user-facing controls for everything above live at `/_authenticated/governance` (sidebar → AI Ops → Governance): kill-switch toggle (with paused-banner in `AppShell`), per-mission token/spend caps with live usage, and the stale-approvals panel (extend / approve / reject). Operator walkthrough + verification checklist: [`../docs/planning/archive/feature-backlog.md`](../docs/planning/archive/feature-backlog.md) §0.6 "How to use / verify".
