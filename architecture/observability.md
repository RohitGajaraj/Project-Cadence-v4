...
- **BYOK (bring your own key), Built.** When a user supplies a provider key (`user_api_keys`) and the model prefix matches a known BYO provider, the chokepoint routes the call to the user's own key and the inference cost moves off our books entirely. Model-agnostic by contract (adding a provider is an adapter, not a call-site change), so BYOK is a first-class path, not a bolt-on.
