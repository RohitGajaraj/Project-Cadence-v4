...
export const listTraces = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let q = context.supabase
      .from("ai_events")
      .select("id,trace_id,parent_event_id,created_at,surface,model,status,total_tokens,est_cost_usd,latency_ms")
    ...
    for (const r of (rows ?? []) as EventRow[]) {
      ...
      const t = byTrace.get(r.trace_id) ?? {
        trace_id: r.trace_id, first_at: r.created_at, last_at: r.created_at,
        surfaces: new Set(), models: new Set(),
        spans: 0, tokens: 0, cost: 0, latency_ms: 0, errors: 0,
        root_surface: null, root_at: r.created_at,
      };
      t.spans += 1;
      ...
      if (r.created_at < t.first_at) { t.first_at = r.created_at; t.root_surface = r.surface; t.root_at = r.created_at; }
    ...
    const traces = [...byTrace.values()]
      ...
      .filter((t) => data.status === "all" ? true : data.status === "error" ? t.errors > 0 : t.errors === 0)
...
export const getTrace = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) =>
    z.object({ traceId: z.string().uuid() }).parse(i),
  )
  .handler(async ({ context, data }) => {
    ...
    const [hitsRes, evalRes] = await Promise.all([
      ids.length
        ? supabase.from("guardrail_hits")
            .select("id,event_id,rule_name,side,action,kind,matched,created_at")
            .in("event_id", ids)
        : Promise.resolve({ data: [] as never[] }),
      ids.length
        ? supabase.from("ai_evals")
            .select("event_id,relevance,groundedness,coherence,hallucination_score,toxicity,pii_risk,judge_rationale")
            .in("event_id", ids)
        : Promise.resolve({ data: [] as never[] }),
    ]);
    ...
    return {
      ...
      events: rows,
      hits: (hitsRes.data ?? []) as { id: string; event_id: string; rule_name: string; side: string; action: string; kind: string; matched: string | null; created_at: string }[],
      evals: (evalRes.data ?? []) as { event_id: string; relevance: number | null; groundedness: number | null; coherence: number | null; hallucination_score: number | null; toxicity: number | null; pii_risk: number | null; judge_rationale: string | null }[],
    };
  });