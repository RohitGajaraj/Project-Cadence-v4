...
export const listPromptTemplates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: tpls, error } = await supabase
      ...
      .select("id,surface,key,name,description,active_version_id,default_version_id,built_in,updated_at")
      .eq("user_id", userId)
      .order("surface", { ascending: true }).order("key", { ascending: true });
...
export const getPromptTemplate = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { template_id: string }) => z.object({ template_id: z.string().uuid() }).parse(d))
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const { data: tpl, error } = await supabase
      .from("prompt_templates").select("*")
      .eq("id", data.template_id).eq("user_id", userId).single();
    if (error) throw new Error(error.message);
    const { data: versions } = await supabase
      .from("prompt_versions")
      .select("id,version,system_prompt,user_template,model,temperature,notes,status,created_at,updated_at")
      .eq("template_id", data.template_id).eq("user_id", userId)
      .order("version", { ascending: false });
    const { data: assignment } = await supabase
      .from("prompt_assignments")
      .select("variant_a_version_id,variant_b_version_id,split_pct,enabled")
      .eq("template_id", data.template_id).eq("user_id", userId).maybeSingle();
...
export const createPromptVersion = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: { template_id: string; base_version_id?: string }) =>
    z.object({ template_id: z.string().uuid(), base_version_id: z.string().uuid().optional() }).parse(d))
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    let base: { system_prompt: string; user_template: string; model: string | null; temperature: number | null } = {
      system_prompt: "", user_template: "", model: null, temperature: null,
    };
    if (data.base_version_id) {
      const { data: b } = await supabase.from("prompt_versions")
        .select("system_prompt,user_template,model,temperature")
        .eq("id", data.base_version_id).eq("user_id", userId).maybeSingle();
      if (b) base = b as typeof base;
    }
    const { data: maxRow } = await supabase
      .from("prompt_versions").select("version")
      .eq("template_id", data.template_id).eq("user_id", userId)
      .order("version", { ascending: false }).limit(1).maybeSingle();
    const nextVersion = ((maxRow?.version as number | undefined) ?? 0) + 1;
    const { data: ins, error } = await supabase.from("prompt_versions").insert({
      template_id: data.template_id, user_id: userId, version: nextVersion,
      system_prompt: base.system_prompt, user_template: base.user_template,
      model: base.model, temperature: base.temperature,
...
export const updatePromptVersion = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const { data: cur, error: gErr } = await supabase.from("prompt_versions")
      .select("status").eq("id", data.version_id).eq("user_id", userId).single();
    if (gErr) throw new Error(gErr.message);
    if (cur.status !== "draft") throw new Error("Only draft versions can be edited. Fork a new draft.");
    const patch: {
      system_prompt?: string; user_template?: string;
    ...
    const { data: upd, error } = await supabase.from("prompt_versions")
      .update(patch).eq("id", data.version_id).eq("user_id", userId)
...
export const publishPromptVersion = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const { error: e1 } = await supabase.from("prompt_versions")
      .update({ status: "published" }).eq("id", data.version_id).eq("user_id", userId);
    if (e1) throw new Error(e1.message);
    const { error: e2 } = await supabase.from("prompt_templates")
      .update({ active_version_id: data.version_id })
      .eq("id", data.template_id).eq("user_id", userId);
    if (e2) throw new Error(e2.message);
    // Point A/B variant_a at the newly active version if assignment exists
    await supabase.from("prompt_assignments")
...
export const setAssignment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const patch: {
      variant_a_version_id?: string | null; variant_b_version_id?: string | null;
      split_pct?: number; enabled?: boolean;
    } = {};
    if (data.variant_a_version_id !== undefined) patch.variant_a_version_id = data.variant_a_version_id;
    if (data.variant_b_version_id !== undefined) patch.variant_b_version_id = data.variant_b_version_id;
    if (data.split_pct !== undefined) patch.split_pct = data.split_pct;
    if (data.enabled !== undefined) patch.enabled = data.enabled;
    const { error } = await supabase.from("prompt_assignments")
      .upsert({ user_id: userId, template_id: data.template_id, ...patch }, { onConflict: "user_id,template_id" });
    if (error) throw new Error(error.message);
    return { ok: true };
  });
