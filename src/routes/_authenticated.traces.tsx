...
import { GitBranch, AlertTriangle, ChevronRight, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/traces")({
  ...
  head: () => ({ meta: [{ title: "Traces · Cadence" }] }),
});
        ...
        <header className="flex items-end justify-between gap-4">
          <div>
            <h1 className="font-display text-3xl tracking-tight flex items-center gap-2">
              <GitBranch className="h-6 w-6 text-violet-400" /> Traces
            </h1>
            <p className="mt-1 text-sm text-muted-foreground">
              Distributed AI call graphs — drill into a single request to see every model hop, tool call, and judge.
            </p>
          </div>
