...
import { Activity, AlertTriangle, Clock, DollarSign, Gauge, Shield, Zap, ChevronRight, X } from "lucide-react";
import { AppShell } from "@/components/cadence/AppShell";
import { listProjects } from "@/lib/projects.functions";
import { getAnalyticsOverview, listAiEvents, getEventDetail, getGuardrailStats } from "@/lib/analytics.functions";
...
function fmtUsd(n: number) { return `$${n.toFixed(n < 0.01 ? 5 : 4)}`; }
...
function AnalyticsPage() {
  ...
  const [tab, setTab] = useState<"overview" | "surface" | "model" | "runs" | "guardrails">("overview");
      ...
      <div className="px-6 lg:px-10 py-8 max-w-[1200px] mx-auto">
        <header className="mb-6 flex items-start justify-between gap-4">
          <div>
            <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground flex items-center gap-2">
              <Activity className="h-3.5 w-3.5 text-violet-300" /> AI Analytics
            </div>
            <h1 className="mt-3 font-display text-4xl tracking-tight">
              Observability for every <span className="neural-text">AI call</span>
            </h1>
            <p className="mt-2 text-sm text-muted-foreground max-w-2xl">
              Tokens, latency, cost, errors, guardrail hits and hallucination scores — across agents, chat,
              copilot, PRDs and discovery. The numbers you need to allocate work to AI with confidence.
            </p>
          </div>
        ...
        {tab === "overview" && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <Kpi icon={AlertTriangle} label="Errors" value={String(s?.errors ?? 0)}
            ...
            <section className="bento p-5 col-span-full">
              <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground mb-3">Daily activity</div>
            ...
            <table className="w-full text-sm">
              <thead className="text-xs text-muted-foreground">
                <tr className="text-left">
                  <th className="py-2 pr-4">Surface</th><th>Runs</th><th>Errors</th>
                  <th>Tokens</th><th>Spend</th>
                </tr>
              </thead>
              <tbody>
        ...
        {tab === "model" && (
          <div className="bento p-4">
            <table className="w-full text-sm">
              <thead className="text-xs text-muted-foreground">
                <tr className="text-left">
                  <th className="py-2 pr-4">Model</th><th>Runs</th>
                  <th>Tokens</th><th>Spend</th>
                </tr>
              </thead>
              <tbody>
          ...
          <div className="bento divide-y divide-white/5">
            {(events.data?.events ?? []).map((e) => (
              <button key={e.id} onClick={() => setOpenId(e.id)}
                ...
                <span className={`text-[10px] uppercase px-1.5 py-0.5 rounded ${e.status === "ok" ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-300"}`}>
                  {e.status}
                </span>
            ...
            {!events.isLoading && (events.data?.events ?? []).length === 0 && (
              <div className="p-6 text-sm text-muted-foreground text-center">No AI events yet — run an agent or chat first.</div>
            )}
          </div>
        )}
        ...
        {tab === "guardrails" && (
          <div className="bento p-4">
            ...
            {(guards.data?.hits ?? []).length === 0 && (
              <div className="text-sm text-muted-foreground">No guardrail hits — your inputs and outputs have been clean.</div>
            )}
            <ul className="space-y-2">
              {(guards.data?.hits ?? []).map((h, i) => (
                <li key={i} className="flex items-center gap-3 text-sm">
                  <span className={`text-[10px] uppercase px-1.5 py-0.5 rounded ${h.action === "block" ? "bg-rose-500/10 text-rose-300" : h.action === "redact" ? "bg-amber-500/10 text-amber-300" : "bg-cyan-500/10 text-cyan-300"}`}>
                    {h.action}
                  </span>
...
function Kpi({ icon: Icon, label, value, tone }: { icon: typeof Activity; label: string; value: string; tone?: "ok" | "warn" }) {
  return (
    <div className="bento p-4">
      ...
      <div className={`mt-2 font-display text-2xl ${tone === "warn" ? "text-amber-300" : ""}`}>{value}</div>
    </div>
  );
}

function DailyBars({ data }: { data: { day: string; runs: number; cost: number }[] }) {
  if (data.length === 0) return <div className="text-sm text-muted-foreground">No activity yet.</div>;
  const max = Math.max(...data.map((d) => d.runs), 1);
  return (
    <div className="flex items-end gap-2 h-32">
      {data.map((d) => (
        <div key={d.day} className="flex-1 flex flex-col items-center gap-1 group">
          <div className="text-[10px] text-muted-foreground opacity-0 group-hover:opacity-100">{d.runs}</div>
          <div className="w-full rounded-t bg-gradient-to-t from-violet-500/40 to-cyan-400/60" style={{ height: `${(d.runs / max) * 100}%` }} />
...
type EventDetailData = {
  event: {
    id: string; created_at: string; surface: string; model: string; via: string; status: string;
    prompt_tokens: number; completion_tokens: number; total_tokens: number;
    est_cost_usd: number; latency_ms: number;
    input_preview: string | null; output_preview: string | null; error_message: string | null;
  } | null;
  eval: {
    hallucination_score: number | null; groundedness: number | null;
    relevance: number | null; coherence: number | null; toxicity: number | null;
    judge_rationale: string | null; unsupported_claims: unknown;
  } | null;
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <div className="mt-1 font-display text-lg">{e.surface} · <span className="font-mono text-sm">{e.model}</span></div>
        <div className="text-xs text-muted-foreground mt-1">{new Date(e.created_at).toLocaleString()} · via {e.via}</div>
      </div>
      ...
      {ev && (
        <div className="bento p-4 space-y-2">
          <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Quality scores</div>
          ...
          {ev.judge_rationale && (
            <div className="text-xs text-muted-foreground italic pt-2 border-t hairline">{ev.judge_rationale}</div>
          )}
        </div>
      )}
      ...
      {data.guardrailHits.length > 0 && (
        <div className="bento p-4 space-y-2">
          <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Guardrails</div>
      ...
      <div className="bento p-4">
        ...
        <pre className="text-xs whitespace-pre-wrap font-mono text-muted-foreground">{e.input_preview ?? "(empty)"}</pre>
      </div>
      <div className="bento p-4">
      ...
      {e.error_message && (
        <div className="bento p-4 border border-rose-500/30">
          ...
          <pre className="text-xs whitespace-pre-wrap font-mono text-rose-200">{e.error_message}</pre>
        </div>
      )}
    </div>
  );
...
function Score({ label, v, invert }: { label: string; v: number | null; invert?: boolean }) {
  ...
  return (
    <div className="text-center">
      ...
      <div className={`text-sm mt-1 ${scoreColor(display)}`}>{v == null ? "—" : (v * 100).toFixed(0) + "%"}</div>
    </div>
  );
}