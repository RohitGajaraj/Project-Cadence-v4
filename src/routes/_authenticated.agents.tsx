...
import { Bot, Sparkles, Send, Clock, Gauge, BookOpen } from "lucide-react";
...
function AgentsPage() {
  ...
  const byoProviders = new Set(
    (keysQ.data?.keys ?? []).map((k: { provider: string }) => k.provider),
  );
  ...
  const modelOptions: { id: string; label: string; provider: string }[] = [
    ...
    { id: "openai/gpt-5-mini", label: "GPT-5 mini (Lovable)", provider: "lovable" },
    {
      ...
      label: "Claude 3.5 Sonnet (your key)",
      provider: "anthropic",
    },
  ...
  const selected =
    (agents.data?.agents ?? []).find((a) => a.id === selectedId) ?? (agents.data?.agents ?? [])[0];
  const selAny = selected as
    | (typeof selected & {
  ...
  const saveSchedule = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(schedule ? "Schedule saved" : "Schedule removed");
    },
    onError: (e: Error) => toast.error(e.message),
  });
        ...
        <div className="px-6 lg:px-10 py-8 max-w-[1400px] mx-auto">
          <header className="mb-8">
            ...
            <p className="mt-2 text-sm text-muted-foreground max-w-xl">
              Specialized agents that ground in your workspace and execute work alongside you. Brief
              any agent — review the output, approve, and ship.
            </p>
          </header>
                  ...
                  return (
                    <button
                      ...
                      className={`relative w-full text-left rounded-lg px-3 py-2.5 flex items-center gap-3 transition ${
                        isActive ? "bg-secondary/60 ring-1 ring-border" : "hover:bg-secondary/30"
                      }`}
                    >
                      {isActive && (
                  ...
                  >
                    <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
                      <span className="inline-flex items-center gap-1.5 whitespace-nowrap">
                        <Sparkles className="h-3 w-3 text-violet-400" /> Brief{" "}
                      ...
                      <label className="ml-auto inline-flex items-center gap-1.5 text-[11px] cursor-pointer select-none">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-xs text-left p-3 text-[11px] leading-snug">
                            ...
                            <Link to="/missions" search={{ tab: "missions" }} className="underline">
                              Missions
                            </Link>{" "}
                    ...
                    <div className="flex justify-end">
                      ...
                      >
                        ...
                        {dispatch.isPending ? "Running…" : "Dispatch"}
                      </button>
                    </div>
                  </form>
                  ...
                  <div className="bento p-4 flex flex-col gap-3">
                    ...
                    {selAny?.last_scheduled_run_at && (
                      <div className="text-[10px] text-muted-foreground">
                        Last autonomous run:{" "}
                        {new Date(selAny.last_scheduled_run_at).toLocaleString()}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {selectedRuns.map((r) => (
                      <div key={r.id} className="bento p-4">
                        ...
                        {r.output && (
                          <div className="mt-3 text-sm whitespace-pre-wrap">{r.output}</div>
                        )}
                      </div>
                    ))}
                  </div>
...
const ARC_BLURB: Record<Arc, string> = {
  ...
  trusted:
    "Confirm-mode tools run inline. Review-mode tools still queue. Day-to-day default once an agent has earned it.",
  ambient:
    "Everything runs inline except hard-locked tools (e.g. calendar.create still confirms). Reserve for top-trust agents.",
};
    ...
    return (
      <Tooltip>
        ...
        <TooltipContent className="max-w-xs text-left">
          No trust data yet. Score appears after the first mission, approval, or eval is recorded
          for this agent.
        </TooltipContent>
      </Tooltip>
    );
    ...
    <Tooltip>
      ...
      <TooltipContent className="max-w-sm text-left space-y-1.5 p-3">
        <div className="font-semibold text-[12px]">
          Trust {trust.score}/100 · {label}
        </div>
        <div className="text-[11px] opacity-80 leading-snug">
          0 = no track record · 50 = neutral · 100 = exceptionally reliable. New agents are pulled
          toward 50 until they have ~10 missions (Bayesian shrinkage).
        </div>
        <div className="pt-1 space-y-0.5 text-[11px]">
          <div>
            Missions <span className="opacity-70">(40%)</span>: {b.missions_completed}/
            {b.missions_total} · {Math.round(b.mission_success_rate * 100)}%
          </div>
          <div>
            Approvals <span className="opacity-70">(30%)</span>: {b.approvals_approved}/
            {b.approvals_total} · {Math.round(b.approval_acceptance_rate * 100)}%
          </div>
          <div>
            Evals <span className="opacity-70">(30%)</span>: {b.evals_total} runs · avg{" "}
            {b.eval_mean_score.toFixed(2)}
          </div>
          <div className="opacity-70">Samples: {b.samples}</div>
        </div>
        <div className="pt-1 text-[10px] opacity-70 italic">
          0.4·mission + 0.3·approval + 0.3·eval, shrunk toward 0.5 when n&lt;10. Suggested arc:{" "}
          {ARC_LABELS[trust.suggested_arc]}.
        </div>
      </TooltipContent>
    </Tooltip>
  );
  ...
  return (
    <div>
      <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.16em] text-muted-foreground">
        ...
        <Tooltip>
          ...
          <TooltipContent className="max-w-sm text-left p-3 text-[11px] leading-snug">
            The trust arc this agent runs at. It composes with each tool's own mode to decide
            whether a call runs inline, queues a confirm, or queues a review. Safety floor:{" "}
            <em>review</em> is sticky and hard-locked tools (e.g. calendar.create) always confirm.
          </TooltipContent>
        </Tooltip>
      ...
      <p className="mt-2 text-[10px] text-muted-foreground leading-relaxed">
        Currently <span className="text-foreground">{ARC_LABELS[current]}</span> —{" "}
        {ARC_BLURB[current]}
      </p>
    </div>
  );
    ...
    <div className="space-y-3">
      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground flex items-center gap-2">
        ...
        <Tooltip>
          ...
          <TooltipContent className="max-w-sm text-left p-3 text-[11px] leading-snug">
            After every clean run, {agentName} writes a one-paragraph lesson to its memory. Future
            runs of the same agent recall the top reflections, so behaviour compounds run-over-run.
          </TooltipContent>
        </Tooltip>
      </div>
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
              {r.metadata?.what_worked && (
                <div className="rounded-md border border-border bg-secondary/30 p-2">
                  <div className="text-[9px] uppercase tracking-[0.18em] text-emerald-300/80">
                    What worked
                  </div>
