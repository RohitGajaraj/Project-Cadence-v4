...
function RoadmapPage() {
  ...
  const rebalance = useMutation({
    ...
    onSuccess: (r) => { inv(); toast.success(r.message ?? "Rebalanced"); },
    onError: (e: Error) => toast.error(e.message),
  });

  const [preview, setPreview] = useState<null | { capacityHours: number; tasks: { title: string; estimate_hours: number; is_deep_work: boolean; priority: string }[] }>(null);
  ...
  for (const p of prds) if (p.opportunity_id) prdByOpp.set(p.opportunity_id, { id: p.id, title: p.title });
      ...
      <div className="px-6 lg:px-10 py-8 max-w-[1500px] mx-auto">
        <header className="mb-8 flex items-end justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Sparkles className="h-3.5 w-3.5" /> {rebalance.isPending ? "Rebalancing…" : "AI rebalance"}
            </button>
            <button
          ...
          <section className="bento p-5 mb-6 border-violet-500/40">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{preview.tasks.length} tasks · ~{preview.tasks.reduce((a, t) => a + (t.estimate_hours || 0), 0).toFixed(1)}h estimated · capacity {preview.capacityHours}h</p>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setPreview(null)} className="rounded-lg border hairline px-3 py-1.5 text-xs hover:bg-secondary">Discard</button>
                <button
            ...
            <ul className="grid sm:grid-cols-2 gap-2 max-h-[360px] overflow-y-auto scrollbar-thin">
              {preview.tasks.map((t, i) => (
                <li key={i} className="rounded-lg border hairline px-3 py-2 text-sm flex items-start gap-2">
                  <span className="text-[10px] uppercase tracking-wider rounded-full bg-secondary px-1.5 py-0.5 mt-0.5">{t.priority}</span>
                  <span className="flex-1">{t.title}</span>
                  <span className="text-[10px] text-muted-foreground tabular-nums whitespace-nowrap">{t.estimate_hours}h{t.is_deep_work ? " · deep" : ""}</span>
                </li>
              ))}
            </ul>
          </section>
                      ...
                      >
                        <div className="flex items-start gap-2">
                          <div className="flex-1 min-w-0">
                            ...
                            {o.problem && <div className="text-xs text-muted-foreground line-clamp-2 mt-0.5">{o.problem}</div>}
                          </div>
                          <div className="text-right shrink-0">
                            <div className="neural-text font-display text-sm tabular-nums">{Number(o.ice_score).toFixed(1)}</div>
                            <div className="text-[9px] text-muted-foreground">I{o.impact}·C{o.confidence}·E{o.ease}</div>
                          </div>
                        </div>
                        {prd && (
                          <Link to="/prds/$id" params={{ id: prd.id }} className="mt-2 inline-flex items-center gap-1 text-[10px] text-violet-300 hover:underline">
                            <FileText className="h-3 w-3" /> {prd.title}
                          </Link>
                        )}
                        <button
                          onClick={(e) => { e.stopPropagation(); setLineage({ id: o.id, title: o.title }); }}
        ...
        <p className="mt-6 text-xs text-muted-foreground inline-flex items-center gap-1.5">
          <Calendar className="h-3 w-3" /> Items in <strong>backlog</strong>, <strong>shipped</strong>, or <strong>dropped</strong> live on the <Link to="/opportunities" className="underline">Opportunities</Link> page.
        </p>
      </div>
      <LineageDrawer
