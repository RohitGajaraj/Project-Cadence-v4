...
import { ArrowLeft, Save, Wand2, CheckCircle2, ListTodo, GitBranch, HelpCircle } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/meetings/$id")({
  ...
  head: () => ({ meta: [{ title: "Meeting · Cadence" }] }),
});
  ...
  return (
    <AppShell projects={projects.data?.projects ?? []}>
      <div className="px-6 lg:px-10 py-8 max-w-[1400px] mx-auto">
        <Link to="/meetings" className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground mb-4">
          ...
          <>
            <header className="mb-6 flex items-end justify-between gap-4 flex-wrap">
              <div>
                ...
                <h1 className="mt-2 font-display text-3xl tracking-tight">{m.title}</h1>
                <p className="text-xs text-muted-foreground mt-1">
                  {m.stakeholder ?? "No stakeholder"} · {new Date(m.start_at).toLocaleString()}
                </p>
              </div>
              {m.processed_at && (
                <span className="inline-flex items-center gap-1.5 text-xs text-emerald-300 rounded-full border border-emerald-500/40 px-2.5 py-1">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <section className="space-y-4">
                {!preview ? (
                  <div className="bento p-8 text-center text-sm text-muted-foreground">
                    Paste a transcript and hit <strong>Extract</strong> to see AI-generated summary, decisions, tasks, and open questions.
                  </div>
                ) : (
                  <>
