...
import {
  listEvalSuites, getEvalSuite, createEvalSuite, updateEvalSuite, deleteEvalSuite,
  createEvalCase, updateEvalCase, deleteEvalCase, runEvalSuiteNow, getEvalRun,
    ...
    <div className="flex h-full">
      <aside className="w-80 border-r border-border flex flex-col">
        <div className="p-4 border-b border-border flex items-center justify-between">
          <div>
            <h2 className="font-semibold flex items-center gap-2"><FlaskConical className="h-4 w-4" /> Eval Suites</h2>
            <p className="text-xs text-muted-foreground">Prompt regression tests</p>
          </div>
          <Button size="icon" variant="ghost" onClick={() => setShowCreate(true)} aria-label="New suite">
        ...
        <div className="flex-1 overflow-auto p-2 space-y-1">
          ...
          {!isLoading && suites.length === 0 && (
            <div className="p-3 text-sm text-muted-foreground">No suites yet. Create one to start.</div>
          )}
          {suites.map((s: any) => (
            <button
              key={s.id}
            ...
            >
              ...
              <div className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5">
                <span>{s.surface}/{s.prompt_key}</span>
                <span>•</span>
      ...
      <main className="flex-1 overflow-auto">
        {selectedId ? <SuiteDetail suiteId={selectedId} onDeleted={() => setSelectedId(null)} /> : (
...
function CreateSuiteDialog({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (b: boolean) => void; onCreated: (id: string) => void }) {
  const qc = useQueryClient();
  const createFn = useServerFn(createEvalSuite);
  const [form, setForm] = useState({ name: "", description: "", target: "chat/default", pass_threshold: 70 });
  const m = useMutation({
    mutationFn: async () => {
      const [surface, prompt_key] = form.target.split("/");
      return createFn({ data: { name: form.name, description: form.description || null, surface, prompt_key, pass_threshold: form.pass_threshold } });
    },
    onSuccess: (row: any) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>New eval suite</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div><Label>Name</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Chat tone regression" /></div>
          ...
          <div>
            ...
            <Select value={form.target} onValueChange={(v) => setForm({ ...form, target: v })}>
              ...
              <SelectContent>
                {SURFACE_KEYS.map((s) => (
                  <SelectItem key={`${s.surface}/${s.key}`} value={`${s.surface}/${s.key}`}>{s.label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={() => m.mutate()} disabled={!form.name || m.isPending}>{m.isPending ? "Creating…" : "Create"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
function SuiteDetail({ suiteId, onDeleted }: { suiteId: string; onDeleted: () => void }) {
  ...
  const run = useMutation({
    ...
    onSuccess: (r: any) => {
      toast.success(`Run complete: ${r.passed}✓ ${r.failed}✗ ${r.errored ? r.errored + " errors" : ""}`);
      refetch();
      qc.invalidateQueries({ queryKey: ["eval_suites"] });
    },
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          {suite.description && <p className="text-sm text-muted-foreground mt-1">{suite.description}</p>}
          <div className="flex items-center gap-2 mt-2 text-xs">
            <Badge variant="outline">{suite.surface}/{suite.prompt_key}</Badge>
            <Badge variant="outline">judge: {suite.judge_model}</Badge>
            <Badge variant="outline">pass ≥ {suite.pass_threshold}</Badge>
            <span className="flex items-center gap-1.5 text-muted-foreground">
              <Switch checked={suite.enabled} onCheckedChange={async (v) => { await updateFn({ data: { suite_id: suiteId, enabled: v } }); refetch(); }} />
...
function CaseList({ suiteId, cases, onChange }: { suiteId: string; cases: any[]; onChange: () => void }) {
  ...
  return (
    <>
      <div className="flex justify-end">
        <Button size="sm" variant="outline" onClick={() => setShowNew(true)}><Plus className="h-3.5 w-3.5 mr-1" />Add case</Button>
      </div>
      {cases.length === 0 && <p className="text-sm text-muted-foreground">No cases yet. Add one to start evaluating.</p>}
      {cases.map((c) => (
        <Card key={c.id}>
          ...
          <CardContent className="text-xs space-y-2">
            <div><span className="text-muted-foreground">Input:</span> <span className="whitespace-pre-wrap">{c.input}</span></div>
            {c.expected && <div><span className="text-muted-foreground">Expected:</span> <span className="whitespace-pre-wrap">{c.expected}</span></div>}
            {c.rubric && <div><span className="text-muted-foreground">Rubric:</span> <span className="whitespace-pre-wrap">{c.rubric}</span></div>}
          </CardContent>
        </Card>
      ))}
      ...
      <Dialog open={showNew} onOpenChange={setShowNew}>
        <DialogContent>
          <DialogHeader><DialogTitle>New eval case</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div><Label>Name</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
            <div><Label>Input (user message)</Label><Textarea rows={4} value={form.input} onChange={(e) => setForm({ ...form, input: e.target.value })} /></div>
            <div><Label>Expected output (optional)</Label><Textarea rows={3} value={form.expected} onChange={(e) => setForm({ ...form, expected: e.target.value })} /></div>
            <div><Label>Rubric (optional)</Label><Textarea rows={2} value={form.rubric} onChange={(e) => setForm({ ...form, rubric: e.target.value })} placeholder="e.g. Must be ≤ 5 lines, no emojis, mentions OKRs" /></div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setShowNew(false)}>Cancel</Button>
            <Button disabled={!form.name || !form.input} onClick={async () => {
              await createFn({ data: { suite_id: suiteId, name: form.name, input: form.input, expected: form.expected || null, rubric: form.rubric || null } });
              setShowNew(false);
      ...
      <button className="w-full text-left" onClick={() => setOpen(!open)}>
        <CardHeader className="pb-3 flex flex-row items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-xs">
            <span className="flex items-center gap-1"><CheckCircle2 className="h-3 w-3 text-emerald-500" />{run.pass_count}</span>
            <span className="flex items-center gap-1"><XCircle className="h-3 w-3 text-destructive" />{run.fail_count}</span>
            {(run.errored ?? 0) > 0 && <span className="flex items-center gap-1"><AlertCircle className="h-3 w-3 text-amber-500" />{run.errored}</span>}
            <Badge variant={pct >= 80 ? "default" : pct >= 50 ? "outline" : "destructive"}>{pct}%</Badge>
            {run.avg_score != null && <span className="text-muted-foreground">avg {Math.round(run.avg_score)}</span>}
          </div>
        </CardHeader>
      </button>
      {open && detail && (
        <CardContent className="space-y-2 border-t pt-3">
          {(detail as any).results.length === 0 && <p className="text-xs text-muted-foreground">No results.</p>}
            ...
            <div key={r.id} className="rounded-md border border-border p-3 text-xs space-y-1">
              <div className="flex items-center justify-between">
                <div className="font-medium flex items-center gap-2">
                  {r.status === "passed" ? <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" /> :
              ...
              {r.case?.input && <div className="text-muted-foreground"><span className="font-medium text-foreground">Input:</span> <span className="whitespace-pre-wrap">{r.case.input}</span></div>}
              {r.actual && <div><span className="font-medium">Actual:</span> <span className="whitespace-pre-wrap">{r.actual}</span></div>}
              {r.judge_reasoning && <div className="text-muted-foreground italic">Judge: {r.judge_reasoning}</div>}
