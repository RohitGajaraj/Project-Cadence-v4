...
function Stat({ value, label, color }: { value: string; label: string; color?: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
      <span className="font-display tabular-nums" style={{ fontSize: 22, color: color ?? "var(--ink)", lineHeight: 1 }}>
        {value}
      </span>
...
function Timeline({ buckets }: { buckets: TimelineBucket[] }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 10, height: 96, padding: "4px 2px" }}>
      {buckets.map((b) => {
        const h = Math.round(((b.decisions + b.learnings) / max) * 76);
        return (
          <div key={b.month} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 5, minWidth: 0 }}>
            <div
              title={`${b.month}: ${b.decisions} decisions (${b.superseded} revised), ${b.learnings} outcomes`}
              style={{ width: "100%", maxWidth: 34, height: Math.max(3, h), borderRadius: 4, background: "var(--ember)", opacity: 0.85, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}
            >
              {b.superseded > 0 ? (
                <div style={{ height: `${Math.round((b.superseded / Math.max(1, b.decisions + b.learnings)) * Math.max(3, h))}px`, background: "var(--ink-faint)", borderRadius: "0 0 4px 4px", opacity: 0.7 }} />
              ) : null}
            </div>
            <span className="mono-label tabular-nums" style={{ fontSize: 8, color: "var(--ink-faint)" }}>
              {b.month.slice(2)}
            </span>
          </div>
        );
...
export function InsightsPanel() {
  ...
  if (q.isPending) {
    return <div style={{ fontSize: 13, color: "var(--ink-subtle)", padding: "28px 0" }}>Reading the brain…</div>;
  }
  if (q.isError) {
    return <div style={{ fontSize: 13, color: "var(--rose)", padding: "28px 0" }}>Could not load insights. {(q.error as Error)?.message}</div>;
  }
  const d = q.data!;
        ...
        {d.insights.map((ins, i) => {
          ...
          return (
            <div
              key={i}
              className="bento"
              style={{ padding: "12px 15px", display: "flex", alignItems: "flex-start", gap: 10, borderLeft: `2px solid ${t.color}` }}
            >
              <t.Icon size={15} strokeWidth={1.9} color={t.color} style={{ flexShrink: 0, marginTop: 1 }} />
              <span style={{ fontSize: 13.5, color: "var(--ink)", lineHeight: 1.5 }}>{ins.text}</span>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div className="bento" style={{ padding: 16 }}>
          ...
          <div style={{ display: "flex", gap: 24 }}>
            ...
            <Stat value={String(d.beliefs.superseded)} label="revised since" color="var(--ink-subtle)" />
          </div>
        ...
        <div className="bento" style={{ padding: 16 }}>
          ...
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
            <Stat value={d.learned.hitRate === null ? "—" : `${d.learned.hitRate}%`} label="hit rate" color="var(--ember)" />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {d.recentLearnings.map((l, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <span
                  className="mono-label"
                  style={{ fontSize: 9, color: VERDICT_COLOR[l.verdict?.toLowerCase()] ?? "var(--ink-subtle)", flexShrink: 0, minWidth: 60, textTransform: "uppercase" }}
                >
                  {l.verdict || "—"}
                </span>
                <span style={{ fontSize: 12.5, color: "var(--ink-muted, #4a443c)", lineHeight: 1.5, flex: 1 }}>
                  {l.summary || "(no summary)"}
                  {l.metricLabel && l.metricValue ? (
                    <span className="mono-label" style={{ fontSize: 9, color: "var(--ink-faint)", marginLeft: 6 }}>
                      {l.metricLabel}: {l.metricValue}
                    </span>
                  ) : null}
                  {l.iceShift !== null && l.iceShift !== 0 ? (
                    <span
                      ...
                      style={{ fontSize: 9, marginLeft: 6, color: l.iceShift > 0 ? "var(--emerald)" : "var(--coral)" }}
                    >
                      ICE {l.iceShift > 0 ? "+" : ""}
                      {l.iceShift}
                    </span>
