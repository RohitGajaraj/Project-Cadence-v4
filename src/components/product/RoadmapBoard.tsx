...
export function RoadmapBoard() {
  ...
  const mFields = useMutation({
    mutationFn: (v: { id: string; outcome: string; measure: string }) =>
      fUpdate({ data: { id: v.id, outcome: v.outcome || null, measure: v.measure || null } }),
    onSuccess: () => {
      setEditId(null);
      setOutcome("");
      setMeasure("");
                  ...
                  {col.key && editId !== it.id && (
                    <div style={{ marginTop: 5 }}>
                      {(it.outcome || it.measure) && (
                        <p style={{ fontSize: 10.5, color: "var(--ink-subtle)", lineHeight: 1.4 }}>
                          {it.outcome}
                          {it.outcome && it.measure ? " · " : ""}
                          {it.measure && (
                    ...
                    <div style={{ marginTop: 6, display: "flex", flexDirection: "column", gap: 5 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                        ...
                        <button
                          ...
                          onClick={() => mFields.mutate({ id: it.id, outcome, measure })}
