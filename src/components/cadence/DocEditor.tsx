...
export function DocEditor({
  ...
  const slashItems = [
    { label: "Heading 1", icon: <Heading1 className="h-4 w-4" />, run: () => { deleteSlashChar(); editor.chain().focus().toggleHeading({ level: 1 }).run(); } },
    { label: "Heading 2", icon: <Heading2 className="h-4 w-4" />, run: () => { deleteSlashChar(); editor.chain().focus().toggleHeading({ level: 2 }).run(); } },
    { label: "Bullet list", icon: <List className="h-4 w-4" />, run: () => { deleteSlashChar(); editor.chain().focus().toggleBulletList().run(); } },
    { label: "Numbered list", icon: <ListOrdered className="h-4 w-4" />, run: () => { deleteSlashChar(); editor.chain().focus().toggleOrderedList().run(); } },
    { label: "Quote", icon: <Quote className="h-4 w-4" />, run: () => { deleteSlashChar(); editor.chain().focus().toggleBlockquote().run(); } },
    { label: "Code block", icon: <CodeIcon className="h-4 w-4" />, run: () => { deleteSlashChar(); editor.chain().focus().toggleCodeBlock().run(); } },
    {
      label: "Figma embed",
  ...
  return (
    <div className="relative">
      <div className="sticky top-0 z-10 flex items-center gap-0.5 border-b hairline bg-background/80 backdrop-blur-md px-1 py-1">
        <Btn title="Heading 1" on={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} active={editor.isActive("heading", { level: 1 })}><Heading1 className="h-4 w-4" /></Btn>
        ...
        <Btn title="Underline" on={() => editor.chain().focus().toggleUnderline().run()} active={editor.isActive("underline")}><UnderlineIcon className="h-4 w-4" /></Btn>
        <div className="w-px h-5 bg-border/60 mx-1" />
      ...
      {slash && (
        <>
          ...
          >
            <div className="px-2 py-1 text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Insert</div>
            {slashItems.map((it) => (
              <button
                key={it.label}
                onMouseDown={(e) => { e.preventDefault(); it.run(); setSlash(null); }}
