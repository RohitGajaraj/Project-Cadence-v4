...
export function VouchersPanel() {
  ...
  const list = useQuery({ queryKey: ["admin-vouchers"], queryFn: () => fList({ data: { active: null } }) });
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div className="bento" style={{ padding: 16 }}>
        <div className="mono-label" style={{ marginBottom: 8 }}>Vouchers · {rows.length}</div>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
          <thead><tr className="mono-label" style={{ color: "var(--ink-subtle)" }}>
            <th style={th()}>Code</th><th style={th()}>Kind</th><th style={th()}>Plan</th>
            <th style={th()}>Credits</th><th style={th()}>Used / Max</th>
            <th style={th()}>Expires</th><th style={th()}>Active</th><th style={th()}></th>
          </tr></thead>
          <tbody>
            {rows.map((v) => (
              <tr key={v.id} style={{ borderTop: "1px solid var(--hairline)" }}>
                ...
                <td style={td()}>
                  ...
                  {v.active ? (
                    <button className="btn btn-sm" style={{ marginLeft: 6 }} onClick={async () => {
                      const ok = await confirm({ title: "Deactivate voucher?", body: `${v.code} can no longer be redeemed. Existing redemptions are kept.`, confirmLabel: "Deactivate", destructive: true });
            ...
            {rows.length === 0 ? <tr><td colSpan={8} style={{ padding: 12, textAlign: "center", color: "var(--ink-subtle)" }}>No vouchers yet.</td></tr> : null}
          </tbody>
        </table>
      </div>
  ...
  return (
    <div className="bento" style={{ padding: 16, display: "grid", gap: 8 }}>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        <input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="LAUNCH50" style={input(140)} />
        <select value={kind} onChange={(e) => setKind(e.target.value as typeof kind)} style={input(140)}>
          <option value="credit_grant">credit_grant</option>
          <option value="plan_upgrade">plan_upgrade</option>
          <option value="signup">signup</option>
        </select>
        <input value={planTier} onChange={(e) => setPlanTier(e.target.value)} placeholder="plan tier" style={input(120)} />
        <input type="number" value={credits} onChange={(e) => setCredits(e.target.value === "" ? "" : Number(e.target.value))} placeholder="credits" style={input(100)} />
        <input type="number" value={maxRedemptions} onChange={(e) => setMaxRedemptions(e.target.value === "" ? "" : Number(e.target.value))} placeholder="max uses" style={input(100)} />
        <input type="number" value={days} onChange={(e) => setDays(e.target.value === "" ? "" : Number(e.target.value))} placeholder="days" style={input(80)} />
        <input value={tag} onChange={(e) => setTag(e.target.value)} placeholder="campaign tag" style={input(140)} />
        <label style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12 }}>
          <input type="checkbox" checked={autoLogin} onChange={(e) => setAutoLogin(e.target.checked)} /> auto-login (signup)
        </label>
        <button className="btn btn-primary btn-sm" disabled={!code || create.isPending}
...
function RedemptionsDrawer({ voucherId, onClose }: { voucherId: string | null; onClose: () => void }) {
  ...
  return (
    <Sheet open={!!voucherId} onOpenChange={(o) => !o && onClose()}>
      <SheetContent side="right" style={{ width: "min(480px, 100vw)", overflow: "auto" }}>
        <SheetHeader><SheetTitle>Redemptions</SheetTitle></SheetHeader>
        <ul style={{ marginTop: 12, padding: 0, listStyle: "none", display: "grid", gap: 6, fontSize: 12.5 }}>
          {rows.length === 0 ? <li style={{ color: "var(--ink-subtle)" }}>No redemptions yet.</li> : null}
          {rows.map((r) => (
            <li key={r.id}>
