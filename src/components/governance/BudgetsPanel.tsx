...
export function BudgetsPanel() {
  ...
  type SurfaceRow = { surface: string; daily_usd_cap: number | string | null; monthly_usd_cap: number | string | null; enabled: boolean; daily_usd_used: number | string; monthly_usd_used: number | string };
  const toggleSurface = useMutation({
    mutationFn: (row: SurfaceRow) => upsertSurface({ data: {
      surface: row.surface,
  ...
  type AlertRow = { id: string; kind: string; scope: string; surface: string | null; window_kind: string; pct: number | string; usd_used: number | string; usd_cap: number | string; created_at: string; acknowledged: boolean };
          ...
          {surfaces.length === 0 ? (
            ...
            <Card key={row.surface}>
              <CardContent className="py-4 grid md:grid-cols-4 gap-4 items-center">
                ...
                <div><div className="text-xs text-muted-foreground mb-1">Today</div><UsageBar used={Number(row.daily_usd_used)} cap={row.daily_usd_cap == null ? null : Number(row.daily_usd_cap)} /></div>
                <div><div className="text-xs text-muted-foreground mb-1">Month</div><UsageBar used={Number(row.monthly_usd_used)} cap={row.monthly_usd_cap == null ? null : Number(row.monthly_usd_cap)} /></div>
