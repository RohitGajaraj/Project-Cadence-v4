    ...
    <div className="space-y-1">
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="text-muted-foreground">
          / ${Number(cap).toFixed(2)} ({p.toFixed(0)}%)
        </span>
      </div>
...
export function BudgetsPanel() {
  ...
  const addSurfaceMut = useMutation({
    ...
    onSuccess: () => {
      toast.success("Surface cap saved");
      setNewSurface({ surface: "chat", daily_usd_cap: "", monthly_usd_cap: "" });
      qc.invalidateQueries({ queryKey: ["budget_overview"] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const surfaces = (data?.surfaces ?? []) as SurfaceRow[];
  type AlertRow = {
    id: string;
    kind: string;
    scope: string;
          ...
          {isLoading ? (
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <Card>
                ...
                <CardContent>
                  <UsageBar
                    used={Number(data?.global?.daily_usd_used ?? 0)}
                    cap={data?.global?.daily_usd_cap ?? null}
                  />
                </CardContent>
              </Card>
              <Card>
                ...
                <CardContent>
                  <UsageBar
                    used={Number(data?.global?.monthly_usd_used ?? 0)}
                    cap={data?.global?.monthly_usd_cap ?? null}
                  />
                </CardContent>
              </Card>
            </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label className="text-xs">Daily USD cap</Label>
                  <Input
                    value={g.daily_usd_cap}
                    onChange={(e) => setG({ ...g, daily_usd_cap: e.target.value })}
                    placeholder="e.g. 5.00"
                  />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs">Monthly USD cap</Label>
                  <Input
                    value={g.monthly_usd_cap}
                    onChange={(e) => setG({ ...g, monthly_usd_cap: e.target.value })}
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    value={g.daily_token_cap}
                    onChange={(e) => setG({ ...g, daily_token_cap: e.target.value })}
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    value={g.monthly_token_cap}
                    onChange={(e) => setG({ ...g, monthly_token_cap: e.target.value })}
                    placeholder="optional"
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    value={g.alert_at_pct}
                    onChange={(e) => setG({ ...g, alert_at_pct: Number(e.target.value) })}
                  />
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                <Plus className="h-4 w-4" /> Add surface cap
              </CardTitle>
            </CardHeader>
            <CardContent className="grid md:grid-cols-4 gap-3 items-end">
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(v) => setNewSurface({ ...newSurface, surface: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-1">
                ...
                <Input
                  value={newSurface.daily_usd_cap}
                  onChange={(e) => setNewSurface({ ...newSurface, daily_usd_cap: e.target.value })}
                  placeholder="optional"
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  value={newSurface.monthly_usd_cap}
                  onChange={(e) =>
                    setNewSurface({ ...newSurface, monthly_usd_cap: e.target.value })
                  }
                  placeholder="optional"
                />
              </div>
              <Button onClick={() => addSurfaceMut.mutate()} disabled={addSurfaceMut.isPending}>
                Save
              </Button>
            </CardContent>
          </Card>
              ...
              <Card key={row.surface}>
                <CardContent className="py-4 grid md:grid-cols-4 gap-4 items-center">
                  <div className="flex items-center gap-3">
                    <Switch
                      checked={row.enabled}
                      onCheckedChange={() => toggleSurface.mutate(row)}
                    />
                    <div>
                  ...
                  <div>
                    ...
                    <UsageBar
                      used={Number(row.daily_usd_used)}
                      cap={row.daily_usd_cap == null ? null : Number(row.daily_usd_cap)}
                    />
                  </div>
                  <div>
                    <div className="text-xs text-muted-foreground mb-1">Month</div>
                    <UsageBar
                      used={Number(row.monthly_usd_used)}
                      cap={row.monthly_usd_cap == null ? null : Number(row.monthly_usd_cap)}
                    />
                  </div>
                  <Button
                    variant="ghost"
                    ...
                    onClick={() => removeSurface.mutate(row.surface)}
                  >
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="text-sm font-medium truncate">
                        {a.scope === "global" ? "Global" : a.surface} · {a.window_kind} {a.kind} at{" "}
                        {Number(a.pct).toFixed(0)}%
                      </div>
                      <div className="text-xs text-muted-foreground">
                        ${Number(a.usd_used).toFixed(4)} of ${Number(a.usd_cap).toFixed(2)} ·{" "}
