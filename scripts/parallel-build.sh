...
BOOTSTRAP='You are a NUMBERED parallel build lane in THIS worktree. Read .remember/LANE.md and docs/operations/autonomous-build-loop.md sections 15-16 plus AGENTS.md section 3, then run as a CONTINUOUS autonomous loop (equivalent to /loop) IN THIS TERMINAL. NO CATEGORY and NO OWNED/FORBIDDEN area: you build the single highest-RANKED free item regardless of theme. Each cycle: git fetch + rebase origin/main; bash scripts/lane.sh reap; SELECT MECHANICALLY by running bash scripts/lane.sh next (it prints the eligible items by Rank) and CLAIM DOWN the list, building the FIRST whose claim wins: for id in $(bash scripts/lane.sh next); do bash scripts/lane.sh claim "$id" <laneN> "<globs>" && break; done. The claim auto-rejects HELD, CONFLICT, and DONE (including chokepoint-pinned files), so you never collide or redo finished work. A partial (half-circle) item is PARTIAL with remaining work: CONTINUE it, never skip it as done or publish-verify-only. After a successful CLAIMED, flip the dashboard row to In Dev and commit + push that claim immediately. Build; gate (bunx tsc --noEmit + bun run build + tests); adversarially review; run the doc-loop; commit explicit paths with a WHY; push. On finish, flip the row done and run bash scripts/lane.sh done <ID> (or mark it partial and bash scripts/lane.sh release <ID>), push immediately, then immediately continue to the next item. If a claimed item has NO autonomous work left (only publish-verify, or the remainder is founder-gated), run bash scripts/lane.sh done <ID> to retire it from the board, then take the next. Never stop after one build. The board is dry ONLY when bash scripts/lane.sh next prints nothing; only then write "board dry - long-polling" and recheck in ~25 min. Never hard-stop; never fence yourself to a category.'
